...
const AllStaffTableRow = ({
  ...
  return (
    staff.username !== "aliadmin" && (
      <tr>
        <td>
          <div className="d-flex align-items-center ">
            {staff.uuid ? staff.uuid : renderEmpty()}
          </div>
        </td>
        <td>
          <div className="d-flex align-items-center ">
            {staff.username ? staff.username : renderEmpty()}
          </div>
        </td>
        <td>
          <div className="d-flex align-items-center">
        ...
        <td>
          <div className="d-flex align-items-center">
            {staff.gender ? (
              <Badge color={`light-${helper.gender.color[staff.gender]}`}>
                {helper.gender.content[staff.gender]}
              </Badge>
            ) : (
              renderEmpty()
            )}
        ...
        <td>
          <div className="d-flex align-items-center">
            {staff.phoneNumber ? staff.phoneNumber : renderEmpty()}
          </div>
        </td>
        <td>
          <div className="d-flex align-items-center">
            <Badge
              color={`light-${helper.status.color[staff.status.toLowerCase()]}`}
            >
              {helper.status.content[staff.status.toLowerCase()]}
            </Badge>
          </div>
        </td>
        <td>
          <div className="d-flex align-items-center">
            {ability.can("manage", "staff-GET") && (
              <Button
                onClick={() => {
                  history.push({
                    pathname: "/userdetails",
                    state: {
                      paramAndType: { param: staff.uuid, type: "staff" },
                      uuid: staff.uuid,
...
const mapDispatchToProps = dispatch => ({
  deleteUser: (userUuid, username, messages) =>
    dispatch(deleteUserAsync(userUuid, username, "staff", messages)),
  clearDeleteUserErrorMessage: () => dispatch(clearDeleteUserErrorMessage()),
})

export default connect(mapStateToProps, mapDispatchToProps)(AllStaffTableRow)
